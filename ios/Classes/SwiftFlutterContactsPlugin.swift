...
public enum FlutterContacts {
    ...
    static func insertGroupInNotDefaultContainer(_ args: [String: Any]) throws -> [String: Any] {
        let group = Group(fromMap: args)
        let newGroup = CNMutableGroup()
        newGroup.name = group.name
        ...
        for container in containers {
            if container.type == CNContainerType.cardDAV && container.identifier != defaultContainerId {
            ...
            DispatchQueue.global(qos: .userInteractive).async {
                do {
                    ...
                    do {
                        let args = call.arguments as! [Any?]
                        let g = args[0] as! [String: Any]
                        let group = try FlutterContacts.insertGroupInNotDefaultContainer(g)
                        result(group)
                    } catch {
                        result(FlutterError(
                            code: "unknown error",
